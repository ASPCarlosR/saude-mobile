...
export default function AvaliacaoElegibilidadeListaScreen() {
  const { width } = useWindowDimensions();
  const isTablet = width >= 768;
  const theme = Colors[useColorScheme() ?? 'light'];
  const styles = getStyles(theme);

  const [lista, setLista] = useState<ElegibilidadeItem[]>([]);
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity style={[styles.card, isTablet && styles.cardTablet]} onPress={() => router.push(`/fichas/avaliacao-elegibilidade?id=${item.id}`)} activeOpacity={0.7}>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()} style={styles.backBtn}><Ionicons name="arrow-back" size={24} color="#EC4899" /></TouchableOpacity>
        <Text style={styles.headerTitulo}>Avaliações de Elegibilidade</Text>
        <View style={{ width: 32 }} />
      </View>
      ...
      {carregando ? (
        <View style={styles.center}><ActivityIndicator size="large" color="#EC4899" /><Text style={styles.loadingTxt}>Buscando avaliações...</Text></View>
      ) : (
        <FlatList key={isTablet ? 'tablet' : 'mobile'} data={lista} keyExtractor={item => item.id} renderItem={renderItem} numColumns={isTablet ? 2 : 1} columnWrapperStyle={isTablet ? styles.rowTablet : undefined} contentContainerStyle={styles.listContainer} ListEmptyComponent={<View style={styles.empty}><Ionicons name="medical-outline" size={48} color="#CBD5E1" /><Text style={styles.emptyTxt}>Nenhuma avaliação encontrada.</Text></View>} />
      )}

      <TouchableOpacity style={styles.fab} onPress={() => router.push('/fichas/avaliacao-elegibilidade')}>
...
const getStyles = (theme: any) => StyleSheet.create({
  // Nota: Os estilos são estruturalmente idênticos aos do Marcador de Consumo
  // Modificamos a cor primária para combinar com a tela de Avaliação (Rosa/EC4899)
  ...
  filterBtn: { width: 44, height: 44, backgroundColor: '#FDF2F8', borderRadius: 8, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#EC4899' },
  ...
  fab: { position: 'absolute', bottom: 24, right: 24, width: 64, height: 64, borderRadius: 32, backgroundColor: '#EC4899', alignItems: 'center', justifyContent: 'center', elevation: 5 },
  ...
  radioBtnOn: { backgroundColor: '#EC4899', borderColor: '#EC4899' },
  ...
  btnAplicar: { flex: 2, padding: 14, borderRadius: 10, alignItems: 'center', backgroundColor: '#EC4899' },
